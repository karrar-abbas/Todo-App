...
class MainActivity : AppCompatActivity() {
   ...
   private var TasksChoice:Int=3
    ...
    fun notifyAdapter(){
        when(TasksChoice){
            0 -> dbManager.choiceStatus(0)
            1 -> dbManager.choiceStatus(1)
            2 -> dbManager.choiceStatus(2)
            else -> dbManager.readData()
        }
